...
class Playable:
    """The Ravelink Playable object which represents all tracks in Ravelink v1.0.0.
    ...
    @property
    def id(self) -> str:
        """Alias for :attr:`identifier` for Client-style code."""
    ...
    def source_name(self) -> str:
        """Alias for :attr:`source` matching Lavalink payload terminology."""
        return self.source

    @property
    def normalized_title(self) -> str:
        """Lowercase title normalized for simple duplicate and recommendation scoring."""
        return " ".join(self.title.lower().split())

    @property
    def requester_id(self) -> object | None:
        """Requester id stored in ``track.extras``, if present."""
        return getattr(self.extras, "requester_id", None)
    ...
    def playlist(self) -> PlaylistInfo | None:
        """Property returning a :class:`ravelink.PlaylistInfo`. Could be ``None``
    ...
    def extras(self) -> ExtrasNamespace:
        """Property returning a :class:`~ravelink.ExtrasNamespace` of extras for this :class:`Playable`.

        You can set this property with a :class:`dict` of valid :class:`str` keys to any valid ``JSON`` value,
        or a :class:`~ravelink.ExtrasNamespace`.

        If a dict is passed, it will be converted into an :class:`~ravelink.ExtrasNamespace`,
        which can be converted back to a dict with dict(...). Additionally, you can also use list or tuple on
        :class:`~ravelink.ExtrasNamespace`.
        ...
        --------
            ...
            .. code:: python
                ...
                track: ravelink.Playable = ravelink.Playable.search("QUERY")
    ...
    def raw_data(self) -> TrackPayload:
        ...
        --------
            ...
            .. code:: python3
                ...
                # Later...
                track: ravelink.Playable = ravelink.Playable(old_data)
    ...
    async def search(
        ...
        .. note::
            ...
            This method differs from :meth:`ravelink.Pool.fetch_tracks` in that it will apply a relevant search prefix
            for you when a URL is **not** provided. This prefix can be controlled via the ``source`` keyword argument.


        .. note::

            This method of searching is preferred over, :meth:`ravelink.Pool.fetch_tracks`.
        ...
        source: :class:`TrackSource` | str | None
            ...
            For basic searches and common plugin sources, see: :class:`ravelink.TrackSource`.
            Otherwise, a ``str`` may be provided for plugin based searches, E.g. "spsearch:" for the
            LavaSrc Spotify based search.

            Defaults to :attr:`ravelink.TrackSource.YouTubeMusic` which is equivalent to "ytmsearch:".
        node: :class:`~ravelink.Node` | None
            An optional :class:`~ravelink.Node` to use when searching for tracks. Defaults to ``None``, which uses
            the :class:`~ravelink.Pool`'s automatic node selection.


        Returns
        -------
        :class:`ravelink.Search`
        ...
        .. code:: python3
            ...
            tracks: ravelink.Search = await ravelink.Playable.search("Ocean Drive")
            if not tracks:
                # No tracks were found...
                ...

            # Search for tracks, with a URL.
            tracks: ravelink.Search = await ravelink.Playable.search("https://www.youtube.com/watch?v=KDxJlW6cxRk")
            ...

            # Search for tracks, using Spotify and the LavaSrc Plugin.
            tracks: ravelink.Search = await ravelink.Playable.search("4b93D55xv3YCH5mT4p6HPn", source="spsearch")
            ...

            # Search for tracks, using Spotify and the LavaSrc Plugin, with a URL.
            tracks: ravelink.Search = await ravelink.Playable.search("https://open.spotify.com/track/4b93D55xv3YCH5mT4p6HPn")
            ...
            ...
            tracks: ravelink.Search = await ravelink.Playable.search("https://open.spotify.com/playlist/37i9dQZF1DWXRqgorJj26U")
            ...


        .. versionchanged:: v1.0.0

            This method has been changed significantly in version ``v1.0.0``. This method has been simplified to provide
            an easier interface for searching tracks. See the above documentation and examples.

            You can no longer provide a :class:`ravelink.Node` to use for searching as this method will now select the
            most appropriate node from the :class:`ravelink.Pool`.
        """
        prefix: str | None = normalize_source_prefix(source)
        check = yarl.URL(query)

        if check.host:
            tracks: Search = await ravelink.Pool.fetch_tracks(query, node=node)
            return tracks
        ...
        tracks: Search = await ravelink.Pool.fetch_tracks(term, node=node)
        return tracks


class Playlist:
    """The Ravelink Playlist container class.

    This class is created and returned via both :meth:`Playable.search` and :meth:`ravelink.Pool.fetch_tracks`.

    It contains various information about the playlist and a list of :class:`Playable` that can be used directly in
    :meth:`ravelink.Player.play`. See below for various supported operations.


    .. warning::

        You should not instantiate this class manually,
        use :meth:`Playable.search` or :meth:`ravelink.Pool.fetch_tracks` instead.
    ...
    .. note::
        ...
        This class can be directly added to :class:`ravelink.Queue` identical to :class:`Playable`. When added,
        all tracks contained in this playlist, will be individually added to the :class:`ravelink.Queue`.
        ...
        **attrs
        ...
        --------
            ...
            .. code:: python3
                ...
                track: ravelink.Playable = playlist[0]
    ...
    @property
    def extras(self) -> ExtrasNamespace:
        """Property returning a :class:`~ravelink.ExtrasNamespace` of extras for this :class:`Playlist`.
        ...
        --------
            ...
            .. code:: python
                ...
                playlist: ravelink.Search = ravelink.Playable.search("QUERY")
...
class PlaylistInfo:
    """The Ravelink PlaylistInfo container class.

    It contains various information about the playlist but **does not** contain the tracks associated with this
    playlist.

    This class is used to provided information about the original :class:`ravelink.Playlist` on tracks.
